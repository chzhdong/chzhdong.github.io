...
本题考虑将矩阵按行或列仅需划分使其差值最小，由于其基于行或列进行划分，所以无需保存所有值，仅需保存对应的行或列的总和即可。之后，获取所有区域的价值综合，并依次按照行或列进行划分，并记录最小的值即可。
...
int main(){
    int n = 0, m = 0;
    int area = 0, num = 0, sum = 0;
    
    std::cin >> n >> m;
    if(n == 1 && m == 1){
        std::cout << "The inputs are illegal." << std::endl;
        return 0;
    }
